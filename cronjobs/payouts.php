...
if ($setting->getValue('disable_manual_payouts') != 1 && $aManualPayouts) {
  ...
  foreach ($aManualPayouts as $aUserData) {
    ...
    if ($bitcoin->validateaddress($aUserData['coin_address'])) {
      ...
      $dMPPayoutLoop = ceil($dMPAmountOriginal / $config['mp_threshold']['max']);
      $dMPPayoutLoopStart = 1;
      $dMPAmountLeft = $dMPAmountOriginal;
      while($dMPAmountLeft > 0) {
        if ($dMPAmountLeft > $config['mp_threshold']['max']) {
          $log->logInfo('Found bigger amount than max payout. Payout will be split into ' . $dMPPayoutLoop .' payouts.');
          $log->logInfo('  Split payout to: ' . $config['mp_threshold']['max'] . ' Left: ' . $dMPAmountLeft . ' Original: ' . $dMPAmountOriginal);
          ...
          $log->logInfo('  Split payout ' . $config['mp_threshold']['max'] . ' is too small. Left: ' . $dMPAmountLeft . ' Original: ' . $dMPAmountOriginal);
          break;
        } else {
...
if ($setting->getValue('disable_auto_payouts') != 1 && $aAutoPayouts) {
  ...
  foreach ($aAutoPayouts as $aUserData) {
    ...
    if ($bitcoin->validateaddress($aUserData['coin_address'])) {
      ...
      $dAPPayoutLoop = ceil($dAPAmountOriginal / $config['ap_threshold']['max']);
      $dAPPayoutLoopStart = 1;
      $dAPAmountLeft = $dAPAmountOriginal;
      while($dAPAmountLeft > 0) {
        if ($dAPAmountLeft > $config['ap_threshold']['max']) {
          $log->logInfo('Found bigger amount than max payout. Payout will be split into ' . $dAPPayoutLoop .' payouts.');
          $log->logInfo('  Split payout to: ' . $config['ap_threshold']['max'] . ' Left: ' . $dAPAmountLeft . ' Original: ' . $dAPAmountOriginal);
          $dAPAmountPayout = $config['ap_threshold']['max'] - $config['txfee_auto'];
